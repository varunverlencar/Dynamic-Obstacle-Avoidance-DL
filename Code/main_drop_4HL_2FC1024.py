...
# graph.write_png('1-4HL-2FCmodel.png')
# plot(model, to_file='1-4HL-2FCmodel.png')
...
ensure_dir(folder)
filepath= folder + "1-1024-4HL-2FC_weights.best.hdf5"
...
fileName = "1-1024-4HL-2FC_accuracy_val.png"
...
plt.xlabel('epoch')
plt.legend(['train', 'test'], loc='lower right')
fileName = "1-1024-4HL-2FC_loss.png"
plt.savefig(folder + fileName, bbox_inches='tight')
plt.close(fig)

folder  = "Files/"
ensure_dir(folder)
with open(folder +"1-1024-Output_4HL-2FC.txt", "wb") as text_file:
	text_file.write("FC=1024 Batch 30 Using Opt=Nadam, lr =  %.8f, decay =  %.8f, reg =  %.8f\n  Validation Error: %.2f%%, Test Error: %.2f%%, for nb_val_samples=%d samples_per_epoch=%d" %(learn_r,dec,reg,100-vscores[1]*100,100-tscores[1]*100,j,i))
