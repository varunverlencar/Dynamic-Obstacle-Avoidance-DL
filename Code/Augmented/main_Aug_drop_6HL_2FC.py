...
ensure_dir(folder)
filepath= folder + "1-Aug-6HL-2FC_weights.best.hdf5"
...
fileName = "1-Aug-6HL-2FC_accuracy_val.png"
...
plt.xlabel('epoch')
plt.legend(['train', 'test'], loc='lower right')
fileName = "1-Aug-6HL-2FC_loss.png"
plt.savefig(folder + fileName, bbox_inches='tight')
plt.close(fig)

folder  = "Files/"
ensure_dir(folder)
with open(folder +"1-Aug-Output_6HL-2FC.txt", "wb") as text_file:
	text_file.write("Using Opt=Nadam,batch =15, (224,224) lr =  %.8f, decay =  %.8f, reg =  %.8f\n  Validation Error: %.2f%%, Test Error: %.2f%%, for nb_val_samples=%d samples_per_epoch=%d" %(learn_r,dec,reg,100-vscores[1]*100,100-tscores[1]*100,j,i))
