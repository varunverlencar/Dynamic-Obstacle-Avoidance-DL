...
if __name__ == "__main__":
    ...
    vfile = 'IMG_6884.MOV'
    cap = cv2.VideoCapture('../../../DatasetVideos/new-Videos/BG3_obj3/' + vfile)
    # Enforce size of frames
    cap.set(3, 160) 
    cap.set(4, 90)

    videoSave = cv2.VideoWriter('testOutput/Videos/new_Output_Aug_' + vfile.split('.')[0] + '.avi', -1, 20.0, (960 ,540))
...
#        start_time = time.clock()
        ...
        im = cv2.resize(frame, (160, 90)).astype(np.float32)
        # im[:,:,0] -= 103.939
        # im[:,:,1] -= 116.779
        # im[:,:,2] -= 123.68
        ...
        cv2.imshow('Augmented-Indoor Obstacle Avoidance',resized)
