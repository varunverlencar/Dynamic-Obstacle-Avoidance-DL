...
samples_per_epoch=2500 #samples_per_epoch
nb_val_samples=1200 #nb_val_samples
...
csv_logger = CSVLogger('Results/Logs/test-training.csv',separator = ',')

folder  = "Weights/"
ensure_dir(folder)
filepath= folder + "test_weights.best.hdf5"
...
vscores = model.evaluate_generator(validation_generator,val_samples = nb_test_samples)
...
plt.grid()
fileName = "test_accuracy_val.png"
plt.savefig(folder + fileName,dpi=300, format='png', bbox_inches='tight')
plt.close(fig)
...
ensure_dir(folder)
with open(folder +"test.txt", "wb") as text_file:
...
cnf_matrix.savefig(folder + 'ConfusionMatrix_test' + '.png', dpi=300, format='png', bbox_inches='tight')
...
plt.savefig(folder + 'test_ClassificationReport.png', dpi=300, format='png', bbox_inches='tight')
plt.close()
...
classifaction_report_csv(report,folder + 'test')
